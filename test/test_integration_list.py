...
class CliIntegrationTestList(IntegrationTestCaseBase):  # pragma: no cover
    ...
    def setUp(self):
        ...
        self.invoke_cli(self.cli_auth_params,
                        ['policy', 'replace', '-b', 'root', '-f',
                         self.environment.path_provider.get_policy_path("list")])
    ...
    def test_list_insecure_list_prints_warning_in_log(self):
        with self.assertLogs('', level='DEBUG') as mock_log:
            ...
            self.assertIn(
                "Warning: Running the command with '--insecure' makes your system vulnerable to security attacks",
                str(mock_log.output))
    ...
    # note: sometimes get timeout need to understand
    @integration_test()
    ...
    def test_list_role_nonexistent_user_returns_forbidden(self):
        output = self.invoke_cli(self.cli_auth_params,
                                 ['list', '-r', f'{self.client_params.account}:user:nonexistinguser'], exit_code=1)
