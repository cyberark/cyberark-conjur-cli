...
"""
CLI User/Host Integration tests
...
# the same build step
class CliIntegrationUserHostTest(IntegrationTestCaseBase):  # pragma: no cover
    capture_stream = io.StringIO()
    def __init__(self, testname, client_params=None, environment_params=None):
        super(CliIntegrationUserHostTest, self).__init__(testname, client_params, environment_params)
    ...
    def setUp(self):
        ...
        self.invoke_cli(self.cli_auth_params,
                        ['policy', 'replace', '-b', 'root', '-f', self.environment.path_provider.get_policy_path("user_host")])

    # *************** TESTS ***************

    @integration_test()
    def test_user_insecure_prints_warning_in_log(self):
