...
from conjur.init.init_command_logic import InitCommandLogic as InitCommandLogic
...
class InitControllerTest(unittest.TestCase):
    ...
    def test_init_constructor(self):
        ...
        mock_init_command_logic = None
        mock_force = False
        InitController(mock_conjurrc_data, mock_init_command_logic, mock_force)

        assert InitController.conjurrc_data == mock_conjurrc_data
        assert InitController.init_command_logic == mock_init_command_logic
    ...
    def test_init_not_trusting_cert_raises_error(self, mock_input):
        ...
        self.init_command_logic.connect = MagicMock(return_value = sock)
        self.init_command_logic.get_certificate = MagicMock(return_value = ["12:AB", "somecertchain"])

        with self.assertRaises(ValueError):
            init_controller = InitController(self.conjurrc_data,self.init_command_logic, self.force_overwrite)
    ...
    def test_init_user_trusts_cert_returns_cert(self, mock_input):
        ...
        self.conjurrc_data.appliance_url = "https://someurl"
        self.init_command_logic.get_certificate = MagicMock(return_value = ["12:AB", mock_certificate])
        init_controller = InitController(self.conjurrc_data,self.init_command_logic, self.force_overwrite)
    ...
    @patch('os.path.join')
    @patch('conjur.init.init_command_logic')
    def test_user_does_not_supply_cert_writes_to_file_called(self, mock_init_command_logic, mock_cert_path):
        self.conjurrc_data.appliance_url="https://someurl"

        mock_init_command_logic.write_certificate_to_file.return_value = None
        mock_cert_path.return_value = "/some/path/somepem.pem"
        InitController.write_certificate(self, mock_init_command_logic, self.conjurrc_data, None, "-----BEGIN CERTIFICATE-----")

        assert self.conjurrc_data.cert_file == mock_cert_path.return_value
        mock_init_command_logic.write_certificate_to_file.assert_called_once()


    @patch('conjur.init.init_command_logic')
    def test_user_supplies_cert_writes_to_file_not_called(self, mock_init_command_logic):
        InitController.write_certificate(self, mock_init_command_logic, self.conjurrc_data, "https://some/cert/path", None)
        mock_init_command_logic.write_certificate_to_file.assert_not_called()
