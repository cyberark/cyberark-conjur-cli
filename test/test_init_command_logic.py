...
class InitCommandLogicTest(unittest.TestCase):
    ...
    def test_certificate_is_written_to_file(self, mock_input):
        with mock.patch.object(self.init_command_logic, '_InitCommandLogic__overwrite_file_if_exists', create=True, return_value=None):
            with patch("builtins.open", mock_open(read_data=MOCK_CERT)) as mock_file:
                with redirect_stdout(self.capture_stream):
                    self.init_command_logic.write_certificate_to_file(MOCK_CERT, "path/to/cert.pem", False)
    ...
    def test_conjurrc_is_written(self):
        with mock.patch.object(self.init_command_logic, '_InitCommandLogic__overwrite_file_if_exists', create=True, return_value=None):
            with patch("builtins.open", mock_open(read_data=EXPECTED_CONFIG)):
                with redirect_stdout(self.capture_stream):
                    self.init_command_logic.write_conjurrc("path/to/conjurrc", self.conjurrc_data, False)
    ...
    def test_user_does_not_want_to_overwrite_raises_exception(self, mock_path, mock_input):
        ...
        with self.assertRaises(Exception):
            self.init_command_logic._overwrite_file_if_exists("somefile", False)
    ...
    def test_cert_error_will_raise_exception(self):
        with patch.object(SSLService, 'get_certificate', side_effect=Exception) as mock_get_cert:
            with self.assertRaises(Exception) as context:
                init_command_logic = InitCommandLogic(self.ssl_service)
                init_command_logic.get_certificate('https://url', None)
    ...
    def test_fingerprint_and_certificate_are_properly_returned(self):
        with patch.object(SSLService, 'get_certificate', return_value=["12:AB", "somecert"]) as mock_ssl:
            mock_init_command = InitCommandLogic(self.ssl_service)
            fingerprint, readable_certificate = mock_init_command.get_certificate("https://someurl", 443)

            self.assertEquals(fingerprint, "12:AB")
            self.assertEquals(readable_certificate, "somecert")
